<?php
  ...
  echo '<div class="mensagem-chat">
       <span  style="background:#273c75;color:white;padding:5px;">'.$nome.'</span>
       <p style="font-size: 25px; margin-top: 10px;">'.$mensagem.'</p>
       </div><!--mensagem-chat-->';
  ...
  $_SESSION['lastIdChat'] = MySql::conectar()->lastInsertId();
      ...
      foreach ($mensagens as $key => $value) {
        ...
        $nomeUsuario = $nomeUsuario->fetch()['nome'];
         echo '<div class="mensagem-chat">
         <span style="background:#FE2E2E;color:white;padding:5px;">'.$nomeUsuario.'</span>
         <p style="font-size: 25px; margin-top: 10px;">'.$value['mensagem'].'</p>
         </div><!--mensagem-chat-->';
